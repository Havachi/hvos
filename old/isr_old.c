...
void isr_insdatall(void) {
	...
	idt_set_gate(32, timer_handler_asm, 0);
...
static void panic_(size_t *ip, size_t *bp, char *fmt, ...) {
	cli();
	va_list args;
	va_start(args, fmt);
...
void isr_common_handler(register_t *frame) {
	...
	if (frame->int_num < 256 && event_handlers[frame->int_num] != NULL)
		return event_handlers[frame->int_num](frame);
	...
	if (frame->int_num < 32) {
        kprintf("AH! UNHANDLED EXCEPTION!\n");
        kprintf("RIP: %016lx\tRBP: %016lx\tRSP: %016lx\n", frame->rip, frame->rbp, frame->rsp);
        kprintf("RAX: %016lx \tRBX: %016lx\tRCX: %016lx\n", frame->rax, frame->rbx, frame->rcx);
        kprintf("RDX: %016lx \tRDI: %016lx\tRSI: %016lx\n", frame->rdx, frame->rdi, frame->rsi);
		kprintf("CS : %04lx\tSS : %04lx\tRFLAGS: %04lx\n", frame->cs, frame->ss, frame->eflags);
		panic_((void *)frame->rip, (void *)frame->rbp, "Unhandled Exception: %s\n", isr_exception_messages[frame->int_num]);
    }
}