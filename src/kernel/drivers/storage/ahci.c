...
hba_port_t *cached_identify_port = NULL;
uint16_t *cached_identify_buffer = NULL;
...
static hba_cmd_header_t *get_port_cmdheader(hba_port_t *port, int slot) {
	uint64_t phys_base = ((uint64_t)port->clbu << 32) | port->clb;
...
static hba_cmd_table_t *get_port_cmdtbl(hba_port_t *port, int slot) {
	...
	uint64_t table_phys_addr = ((uint64_t)cmd_header->ctbau << 32) | cmd_header->ctba;
...
void ahci_port_rebase(hba_port_t *port, int portno) {
	...
	for (int i = 0; i < 32; i++) {
		...
		cmd_header->prdtl = 8;
		cmd_header->ctba = (uint32_t)(cmdtbl_addr & 0xFFFFFFFF);
		cmd_header->ctbau = (uint32_t)((cmdtbl_addr >> 32) & 0xFFFFFFFF);
	}
	start_cmd(port);
}
...
static bool ahci_io_sectors(hba_port_t *port, uint32_t startl, uint32_t starth, uint32_t count, void *buffer, bool is_write, uint8_t ata_command) {
	...
	uint64_t raw_phys_addr = (uint64_t) VIRT_TO_PHYS(buffer);
	void *dma_buffer = buffer;
	bool using_bounce_buffer = false;

	if (raw_phys_addr > 0xFFFFFFFFULL) {
		size_t total_bytes = (size_t)count * SECTOR_SIZE;
		dma_buffer = kmalloc_dma32(total_bytes);
		if (!dma_buffer) {
			printf("[AHCI] Failed allocating DMA32 buffer\n");
			return false;
		}
		using_bounce_buffer = true;
		if (is_write) {
			memcpy(dma_buffer, buffer, total_bytes);
		}
	}

	uint64_t current_buf_addr = (uint64_t) VIRT_TO_PHYS(dma_buffer);
	...
	if (spin == 1000000){
		...
		if (using_bounce_buffer) kfree_dma32(dma_buffer);
		return false;
	}
	port->ci = 1 << slot;
	...
	if(using_bounce_buffer) {
		if(!is_write) {
			memcpy(buffer, dma_buffer, (size_t)count * SECTOR_SIZE);
		}
		kfree_dma32(dma_buffer);
	}
	return true;
}
...
void ahci_probe_port() {
	...
	while(i < 32) {
		if (pi & 1) {
			...
			switch (dt) {
				case AHCI_DEV_SATA:
					storage_register_ahci_drive(&mem->ports[i]);
...
int support_64_dma() {
	return ((hba_cap_t)ahci_mem->cap).s64a;
}

int ahci_read_blocks(block_device_t *dev, uint64_t lba, uint32_t count, void* buffer){
	uint64_t sector_addr = CALC_LBA_DIRECT(lba, dev->block_size, SECTOR_SIZE);
	uint64_t sector_count = (dev->block_size * count) / SECTOR_SIZE;
	if (dev->priv_data == NULL){
		return -1;
	}
	hba_port_t *port = (hba_port_t *)dev->priv_data;
	if (
		ahci_read(port, ((uint32_t)(sector_addr & 0xFFFFFFFF)), ((uint32_t)((sector_addr >> 32) & 0xFFFFFFFF)), sector_count, (uint16_t *)buffer)
	){return 0;}
	return -1;
}
...
int ahci_write_blocks(block_device_t *dev, uint64_t lba, uint32_t count, const void* buffer){
	...
	hba_port_t *port = (hba_port_t *)dev->priv_data;
	if (
		ahci_write(port, ((uint32_t)(sector_addr & 0xFFFFFFFF)), ((uint32_t)((sector_addr >> 32) & 0xFFFFFFFF)), sector_count, (uint16_t *)buffer)
	) {
		return 0;
	} 
	return -1;
...
static uint16_t *ahci_run_identify(hba_port_t *port) {
	if (
		cached_identify_buffer != NULL &&
		memcmp(cached_identify_port, port, sizeof(hba_port_t)) == 0
	){
		return cached_identify_buffer;
	}
	...
	cached_identify_buffer = identify_buf;
	cached_identify_port = port;
	return identify_buf;
}

static uint64_t ahci_get_phys_sector_size(hba_port_t *port) {
	uint16_t *identify_buf = ahci_run_identify(port);
	uint16_t w106 = identify_buf[106];
	if ((w106 & 0xC000) != 0x4000){
		return 512;
	} else if ((w106 & 0xC000) == 0x4000) {
		switch (w106 & 0x3000) {
			case 0x0000:
				return 512;
			case 0x2000:
				return (1 << (w106 & 0xF)) * 512;
			case 0x3000:
			case 0x1000:
				return ((uint32_t)identify_buf[117] << 16 | (uint32_t)identify_buf[118]) * 2;
		}
	}
	return 512;
}

static uint64_t ahci_get_logical_sector_size(hba_port_t *port) {
	uint16_t *identify_buf = ahci_run_identify(port);
	uint16_t w106 = identify_buf[106];
	if ((w106 & 0xC000) != 0x4000){
		return 512;
...
int ahci_get_drv_stat(block_device_t *dev, void *buffer) {
	...
	if (buffer == NULL)
		return -1;
	blkdev_stat_t *stat = buffer;
	stat->total_nb_lsect = ahci_get_sector_count(port);
	stat->physical_sector_size = ahci_get_phys_sector_size(port);
	stat->logical_sector_size = ahci_get_logical_sector_size(port);
	return true;
}

void *ahci_alloc_dma_buffer(size_t size, uintptr_t *out_phys_addr) {
	if (!ahci_mem || size == 0) return NULL;

	void *virt_addr = NULL;
	uintptr_t phys_addr = 0;

	if (support_64_dma()) {
		virt_addr = kmalloc(size);
		if (virt_addr) {
			phys_addr = VIRT_TO_PHYS(virt_addr);
		}
	} else {
		virt_addr = kmalloc_dma32(size);
		if (virt_addr) {
			phys_addr = VIRT_TO_PHYS(virt_addr);
			if (phys_addr > 0xFFFFFFFFULL) {
				kfree_dma32(virt_addr);
				return NULL;
			}
		}
	}
